...
void bookAddedObserver(const Collection<Book>& theCollection,
	const Book& theBook)
{
	std::cout << "Book \"" << theBook.m_title() << "\" added!\n";
}
...
{
	std::cout << "Movie \"" << theMovie.m_title()
...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < 7; i++)
		...
		if (library[i].country() == "US")
			library[i].m_price() *= usdToCadRate;
		else if (library[i].country() == "UK" && library[i].m_year() >= 1990 && library[i].m_year() <= 1999)
			library[i].m_price() *= gbpToCadRate;
	}(i);
	...
	if (argc > 2) {
		...
		do {
			...
			if (file) {
				if (strData[0] != '#') {
					movies[index++] = sdds::Movie(strData);
				}
			}
		} while (file && index < 5);
		file.close();
