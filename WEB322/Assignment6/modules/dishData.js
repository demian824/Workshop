...
const menuModel = mongoose.model("Menu", dishSchema);
...
module.exports.getTopMeal = function() {
    ...
    for (var i = 0; i < dishes.length; i++) {
        ...
        if (dishes[i].rate < dishes[j].rate) {
            temp = dishes[i];
            dishes[i] = dishes[j];
            dishes[j] = temp;
        }
...
module.exports.returnCategory = function(arrary){
    return arrary[0].category.toUpperCase();
}
...
module.exports.findMenu = function(menuname) {
    menuModel.findOne({name: menuname})
    .exec()
    .then((err, item)=> {
        console.log(item);
        return item;
